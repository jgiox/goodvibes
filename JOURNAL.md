...
- Template CI must stay green on an empty project: guard Node steps on `hashFiles('package.json')` and Python steps on `hashFiles('pyproject.toml', 'requirements.txt')`; the template `dependabot.yml` configures only `github-actions` (npm, pip and uv are an opt-in block in getting-started) until `init` tailors it by project type.
