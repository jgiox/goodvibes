...
Always run from `packages/pip/`, not the repo root. `pytest-mock` lives in pip's venv:
```
cd packages/pip && uv run pytest tests/
```
Running `uv run pytest packages/pip/tests/` from the repo root gives a silent
"fixture 'mocker' not found" error — misleading, not a missing dependency.

### Template sync (npm package)
`packages/npm/templates/` is gitignored — it is a prebuild artifact generated by:
```
cd packages/npm && npm run prebuild
```
After adding any file to `templates/` (repo root), run prebuild before building or
publishing, or the npm package ships without the new file. Tests run against the repo-root
`templates/` in dev mode and will pass regardless, masking the missing sync.

### The `tmp_dir` pytest fixture
`tmp_dir` returns `tmp_path` directly — it is NOT a subdirectory. Tests that need a file
"outside" the destination must create a sibling: `outside = tmp_path / "external" / "file.txt"`.
Writing `tmp_path / "outside.txt"` places the file inside `tmp_dir`, making "not exists"
assertions impossible to pass.
<!-- GSD:conventions-end -->
