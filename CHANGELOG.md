...
- `git push --force-with-lease` now asks instead of being refused: the deny rule `Bash(git push --force*)` also matched it. Every other force-push form (`--force`, `-f`, `+branch`, anywhere in the command) is still refused. Projects set up with 1.10.0 or earlier keep the old deny rules after `goodvibes update`; delete `Bash(git push --force*)` and `Bash(git push * --force*)` from `permissions.deny` to get the question
