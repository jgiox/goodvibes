...
- goodvibes never runs a program found through a `PATH` entry that is `.`, empty, relative or inside the project, so a cloned repo cannot plant its own `git`, `claude` or `headroom` for `goodvibes init`, `update` or `doctor` to run (macOS and Linux; Windows already skipped the current folder)
...
- The "Git commit check skipped" line prints `?` for terminal escape codes in a repo's `core.hooksPath` value, and pip `init` prints its global setup list as plain text, never as Rich markup
...
- The npm and pip CLIs now print the same words for `update`, `upgrade` and every help text. `update` shows its plan also with `--force`, leaves out empty groups instead of "(0): (none)", asks "... and apply N change(s) to your Claude Code settings?" (or only "Apply N change(s) to your Claude Code settings?" when the folder has no project setup), and ends with "Applied N file(s). Skipped M user-modified file(s)." and "Done!"; a CLAUDE.md that could not be updated is no longer counted as applied. The "Upgrade did not take effect" message names the npm and the Python fix, and pip no longer prints a second "goodvibes update" heading. pip: `-h` works, `-V` shows the version, `--version` prints only the number (it printed "goodvibes 1.10.0"), running `goodvibes` with no command shows the help, and the shell-completion options are gone. npm: an unknown command or option exits 2 instead of 1, as in pip. `init`'s next step 3 now reads "Start coding: CLAUDE.md rules are already active"
...
- The `.github/dependabot.yml` that goodvibes adds now matches your project: GitHub Actions always, plus npm when there is a `package.json`, uv when there is a `uv.lock`, or else pip when there is a `requirements.txt` or `pyproject.toml`. It used to list npm and pip in every project, and Dependabot fails every week for a package manager whose files a project does not have. `goodvibes update` rewrites an unedited copy of the file the same way, and adds a package manager you start using later; an edited copy is kept. The getting-started guide shows the block to add one by hand
...
- Windows: goodvibes no longer runs a program that a cloned repo ships in the project folder (for example a `git.cmd` or `headroom.cmd`) instead of the real one. Windows looks in the current folder before PATH; every command goodvibes runs now sets `NoDefaultCurrentDirectoryInExePath`, and pip also looks the program up on PATH itself, which also lets it run npm-installed tools such as `claude.cmd`. A `headroom` found inside the project folder is never registered as your user-wide MCP server. The context7 registration (`claude mcp list` and `claude mcp add`), pip's checks for `goodvibes` on PATH and pip's `upgrade` re-run follow the same rule. Not yet tested on a Windows machine
- `goodvibes init` and `update` no longer write the git commit check through a `.git/hooks` folder that is a link (a cloned or tampered repo could point it anywhere). They print "Git commit check skipped: .git/hooks is a link or points outside this repository's git folder, so goodvibes left it alone.", and `goodvibes doctor` shows the check as not managed
...
- Read guard: the secret-file check ignores capital letters and path noise (`.ENV`, `ID_RSA`, `~/.aws//credentials`, `~/.aws/./credentials`), blocks unquoted patterns that could match a secret file (`cat .env*`, `.e?v`, `.[e]nv`, `*.pem`, `id_*`), checks every path of a Kiro `fs_read`, reads `\u00XX` escapes in the tool's JSON, and counts `id_dsa` as a key. It now also checks search and bulk-read tools for secret files: Claude Code and Copilot `Grep`, Gemini CLI `grep_search` and `read_many_files`, and Devin `grep` (a search of a big file is still fine)
- Claude Code deny rules also cover `.env` files in subfolders (`.env.local`, `.env.*.local`, `.env.development`, `.env.dev`, `.env.production`, `.env.prod`, `.env.staging`, `.env.test`) and `id_ecdsa` and `id_dsa` keys; `.env.example` stays readable
- The git commit check runs its git commands with `core.fsmonitor` off and `safe.bareRepository=explicit`, so a cloned repository's config cannot make it start a program
- Journal check: a tool payload spread over several lines (or with Windows line endings) no longer lets every commit through. It also lets through `git commit ... JOURNAL.md` (with or without `-i`/`--only`) when JOURNAL.md has changes, commits during a cherry-pick or revert, and `git commit --help`, `-h` and `--dry-run`; it recognises `/usr/bin/git`, `\git` and `"git"`; and it checks the folder that Gemini CLI (`dir_path`) or Windsurf (`cwd`) says the command runs in
- Read guard: a GitHub Copilot `view_range` counts as a line range, relative paths follow Gemini CLI's `dir_path`, and the size check stops after 50 files per command, so a command naming thousands of files no longer runs past the tools' hook time limits (every file is still checked for secrets)
- `git push --force-with-lease` now asks instead of being refused: the deny rule `Bash(git push --force*)` also matched it. Every other force-push form (`--force`, `-f`, `+branch`, anywhere in the command) is still refused. `goodvibes update` removes the old rules `Bash(git push --force*)` and `Bash(git push * --force*)` where goodvibes installed them, in the project and in `~/.claude/settings.json`; a copy you wrote yourself is kept
- The shipped project settings no longer allow `Write(**)`, which Claude Code ignores and warns about at startup (`Edit(**)` covers writing files); `goodvibes update` removes it from project settings
...
- A `.goodvibes.json` whose entries have the wrong type (`"files": []`, `"managed": 5`, an unknown `scope`) now stops init and update with a message that names the entry, before anything is written. pip used to crash with a traceback, sometimes after writing files, and npm split a text `managed` value into single characters
- pip `goodvibes update` no longer crashes on a JSON `null` in a merged file (`"hooks": null`, `"permissions": null`, `"mcpServers": null`, `"servers": null`), in the project or in `~/.claude/settings.json`; it merges like npm and creates the missing part
- pip `goodvibes update` now asks before adding new files, like npm, and the question says how many it will add
- pip `goodvibes update` fills an empty `"context7": {}` entry with the goodvibes fields, like npm
- npm global setup no longer lists every unchanged rules and skills file as "written" on each run, so `goodvibes update` stops asking when nothing in the Claude config changes
